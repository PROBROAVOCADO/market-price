...
function 市場卡(m, rows) {
  ...
  return `<div class="mk ${m.cls}">
    ...
    <div class="bandKey">
      <span>近 ${S.days} 日加權均價 <b class="num">${錢(wavg)}</b></span>
...
function 設定畫面() {
  ...
  box.innerHTML = `
    ...
    <div class="setRow">
      <h3>上價、中價、下價</h3>
      <p>把當日成交依價位分成三段後，各段的平均價。<b>上價</b>是高價那一段、<b>下價</b>是低價那一段。</p>
    </div>
    <div class="setRow">
      <h3>均價不是三者相加除以三</h3>
      <p>均價是當日全部成交的加權平均。若某天有一批大量低價貨進場，均價會被拉低，但中價可能幾乎不動。</p>
    </div>
    <div class="setRow">
      <h3>期間均價用加權算</h3>
      <p>卡片上的「近 N 日加權均價」以交易量加權，不是把每日均價再平均一次。量小的日子不會和量大的日子等重。</p>
    </div>
